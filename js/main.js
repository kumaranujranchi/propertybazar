  ...
  navLinks.forEach((link) => {
    link.addEventListener("click", (e) => {
      ...
      if (type && window.location.pathname.includes("properties.html")) {
        ...
        const selCity = localStorage.getItem("selectedCity");
        if (selCity && selCity !== "Select City")
          newUrl.searchParams.set("city", selCity);
...
function initTopCitiesTabs() {
  ...
  const CITY_DATA = {
    Bangalore: {
      cols: [
        ...
        {
          title: "House for Sale in Bangalore",
          items: [
            "Whitefield",
            "HSR Layout",
            "JP Nagar",
    ...
    Mumbai: {
      cols: [
        ...
        {
          title: "House for Sale in Mumbai",
          items: [
            "Bandra",
            "Juhu",
            "Powai",
    ...
    Hyderabad: {
      cols: [
        ...
        {
          title: "House for Sale in Hyderabad",
          items: [
            "Banjara Hills",
            "Jubilee Hills",
            "Gachibowli",
    ...
    Chennai: {
      cols: [
        ...
        {
          title: "House for Sale in Chennai",
          items: [
            "Velachery",
            "Adyar",
            "Anna Nagar",
    ...
    Kolkata: {
      cols: [
        ...
        {
          title: "House for Sale in Kolkata",
          items: [
            "Alipore",
            "Ballygunge",
            "Salt Lake",
        ...
        {
          title: "Plots in Kolkata",
          items: [
            "New Town outskirts",
            "Howrah outskirts",
            "Barasat",
            "Basirhat",
            "Bidhannagar periphery",
            "Joka outskirts",
            "Raigunj",
            "Barrackpore",
            "Palta",
    ...
    "Navi Mumbai": {
      cols: [
        ...
        {
          title: "House for Sale in Navi Mumbai",
          items: [
            "Vashi",
            "Kharghar",
            "Panvel",
  ...
  function encodeLocality(s) {
    return encodeURIComponent(s);
  }

  function renderCityGrid(city) {
    const data = CITY_DATA[city] || CITY_DATA["Bangalore"];
    const html = data.cols
      .map((col) => {
        const items = col.items
          .map(
            (it) =>
              `<li><a href="properties.html?city=${encodeLocality(city)}&locality=${encodeLocality(it)}">${col.title.includes(city) ? it : it}</a></li>`,
          )
          .join("\n");
        return `
...
function initSearchTabs() {
    ...
    tab.addEventListener("click", () => {
      ...
      navLinks.forEach((link) => {
        ...
        link.classList.toggle(
          "active",
          linkUrl.searchParams.get("type") === type,
        );
      });
  ...
  if (searchForm) {
    searchForm.addEventListener("submit", (e) => {
      ...
      const type =
        document.querySelector(".search-tab.active")?.dataset?.type || "buy";
...
async function initCityAutocomplete() {
  ...
  dropdown.style.cssText =
    "position:absolute;background:#fff;border:1px solid #E5E7EB;border-radius:8px;box-shadow:0 8px 24px rgba(0,0,0,0.12);width:100%;z-index:100;top:calc(100% + 4px);left:0;overflow:hidden;display:none;max-height:240px;overflow-y:auto;";
...
function initAnimations() {
  ...
  document
    .querySelectorAll(
      ".property-card, .city-card, .prop-type-card, .project-card, .testimonial-card, .team-card",
    )
    .forEach((el) => {
      el.style.opacity = "0";
...
function initEMICalc() {
  ...
  function calcEMI() {
    ...
    if (tenureVal)
      tenureVal.textContent = parseFloat(tenureInput.value) + " yrs";
  }
...
function initFilterPanel() {
  ...
  if (sortSelect)
    sortSelect.addEventListener("change", renderFilteredProperties);
...
function filterProperties() {
  activeFilters.types = [
    ...document.querySelectorAll(
      '.filter-chips[data-type="propType"] .filter-chip.active',
    ),
  ].map((c) => c.dataset.val);
  activeFilters.bhks = [
    ...document.querySelectorAll(
      '.filter-chips[data-type="bhk"] .filter-chip.active',
    ),
  ].map((c) => parseInt(c.dataset.val));
  activeFilters.statuses = [
    ...document.querySelectorAll(
      '.filter-chips[data-type="status"] .filter-chip.active',
    ),
  ].map((c) => c.dataset.val);
  renderFilteredProperties();
}
...
function renderFilteredProperties() {
  ...
  const tab =
    document.querySelector(".search-tab.active")?.dataset?.type || "buy";
  let filtered = window.properties.filter(
    (p) => p.type === tab || tab === "all",
  );
  ...
  if (activeFilters.types.length) {
    filtered = filtered.filter((p) => {
      ...
      return activeFilters.types.some((t) => {
        ...
        if (filterStr === "plot")
          return typeStr.includes("plot") || typeStr.includes("land");
        if (filterStr === "pg") return typeStr.includes("pg");
        return typeStr.includes(filterStr);
      });
    });
  }
  if (activeFilters.bhks.length)
    filtered = filtered.filter((p) => activeFilters.bhks.includes(p.bhk));
...
function buildPropertyCardHTML(p) {
  ...
  <div class="property-card" onclick="handlePropertyListingClick(event, '${detailUrl}')">
    ...
    <div class="prop-body">
      ...
      <div class="prop-price" style="font-size:13px;color:var(--text-muted);margin-bottom:6px">${p.priceDisplay} <span class="prop-per">· ₹${p.price_per_sqft?.toLocaleString("en-IN")}/sqft</span></div>
      <div class="prop-location">📍 ${p.location}</div>
      <div class="prop-specs">
        ${
          p.specs
            ? p.specs
                .map((s) => `<div class="prop-spec">${s.icon} ${s.label}</div>`)
                .join("")
            : `
...
function initPostSteps() {
  ...
  function goToStep(n) {
    ...
    try {
      if (window.applyPropertyTypeUX)
        setTimeout(window.applyPropertyTypeUX, 30);
    } catch (e) {
      /* ignore */
    }
  }
  ...
  function validateStep(index) {
    ...
    formGroups.forEach((group) => {
      ...
      if (label && label.innerText.includes("*")) {
        ...
        if (input) {
          ...
          if (
            id === "descriptionInput" ||
            (label && /Description/i.test(label.innerText))
          )
            minLen = 20;
          else if (
            /pin|pincode|pin code/i.test(label.innerText) ||
            /pin/i.test(id)
          )
            minLen = 6;
          else if (/name/i.test(label.innerText) || /name/i.test(id))
            minLen = 2;

          if (val.length < minLen) {
            isValid = false;
            const reason =
              val.length === 0 ? "Required" : `Too short (min ${minLen})`;
    ...
    if (!isValid && invalidList.length) {
      ...
      summary.style =
        "background:#fff7f6; border:1px solid #ffdddd; color:#b91c1c; padding:10px 12px; border-radius:8px; margin-bottom:12px; font-size:13px;";
  ...
  document.querySelectorAll(".form-type-option").forEach((opt) => {
    opt.addEventListener("click", () => {
      ...
      const gridTitle = opt
        .closest("div")
        .querySelector(".filter-group-title")?.innerText;
...
window.showToast = function (message, type = "success") {
  ...
  const containerSelector =
    type === "warning" ? ".toast-container.warning" : ".toast-container";
  let container = document.querySelector(containerSelector);
  if (!container) {
    container = document.createElement("div");
    container.className =
      "toast-container" +
      (type === "warning" ? " warning bottom centered" : "");
  ...
  const actionHtml =
    type === "warning" ? `<button class="toast-action">Got it</button>` : "";
  toast.innerHTML = `
